...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Normalize tor-nontor to work with FEMaFS and OPF")
    requiredNamed = parser.add_argument_group('required named arguments')
    requiredNamed.add_argument('-ta', '--train', nargs=1, type=str, action='store',
                               help="<Required> tor-nontor train data ",
                               required=True)
    requiredNamed.add_argument('-te', '--test', nargs=1,
                               help="<Required> tor-nontor test data ",

...
void computeDistanceDatasetFromSample(FEMDataset* dataset, FEMSample sample)
{
    ...
    for (i = 0; i < dataset->number_of_samples; i++) {
        dataset->samples[i].dist
            = distanceFEMSample(dataset->samples[i], sample);
    }
    return;
}

void closerSamplesPerClass(FEMDataset dataset, FEMSample sample,
    FEMDataset* closer, int number_of_points, int class)
{
    int i = 0, j = 0;
    double* dist;
    int* index;
    ...
    alloc_FEMDataset(closer, dataset.number_of_classes, number_of_points,
    ...
    if (count < number_of_points) {
        fprintf(stderr,
            "\nERROR: CloserFEMSampleByClass(). Illegal number of points: %d: "
            "%d class:%d",
            number_of_points, count, class);
        exit(1);
    }
...
void mergeCloserFEMSamplesToDataset(FEMDataset* dataset, FEMDataset samples[],
    int number_of_classes, int number_of_points, int number_of_features)
{
    int i = 0, j = 0, count = 0;
    int total_points = number_of_classes * number_of_points;

    alloc_FEMDataset(
        dataset, number_of_classes, total_points, number_of_features);

    for (i = 0; i < number_of_classes; i++) {
        for (j = 0; j < number_of_points; j++) {
            if (count > total_points) {
                fprintf(stderr,
                    "\n ERRO: mergeCloser -> count (%d) > total (%d)", count,
                    total_points);
                exit(1);
            }
...
int invert_matrix(double** mat, double** inv, int n)
{
    ...
    if (*tmp == NULL) {
        fprintf(stderr,
            "\nERROR: - FEMUtils.c  - invert_matrix(%d,%d) - ALLOCATION ", n,
            n);
        exit(1);
    }
        ...
        if (tmp[i][i] == 0.0) {
            ...
            if (finder < n)
                for (col = 0; col < n; ++col) {
                    tmp[i][n]
                        += tmp[finder]
                              [col]; // temp[i*N + col] += temp[finder*N+col];
                    inv[i][col]
                        += inv[finder]
                              [col]; // inv[i*N+col] += inv[finder*N+col];
                }
        }
        if (finder < n) {
            ...
            for (j = i + 1; j < n; ++j) {
                ...
                for (col = 0; col < n; ++col) {
                    tmp[j][col] += (tmp[i][col]
                        * m); // temp[j*N+col] += ( temp[i*N+col] * m );
                    inv[j][col] += (inv[i][col]
                        * m); // inv[j*N+col] += ( inv[i*N+col] * m );
                }
            }
        } else {
            printf("\nMatrix uninvertable.\n");
    ...
    if (finder < n)
        for (i = 0; i < n; ++i)
            for (j = i + 1; j < n; ++j) {
                ...
                for (col = 0; col < n; ++col) {
                    tmp[i][col] += (tmp[j][col]
                        * m); // temp[i*N+col] += ( temp[j*N+col] * m );
                    inv[i][col] += (inv[j][col]
                        * m); // inv[i*N+col] += ( inv[j*N+col] * m );
                }
            }

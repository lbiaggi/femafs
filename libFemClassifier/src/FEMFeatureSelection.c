...
void getMinMaxFeature(FEMDataset *dataset, int feat_id, double *min,
                      double *max) {
  int i;
...
#pragma omp for
  ...
  for (i = 0; i < dataset_train->number_of_features; i++) {
    fprintf(stderr, "\n[BEST %d] => feat:%d score:%.17lf!", i, feat_id[i],
            prob_feat_discrepancy[i]);
  }
  createDatasetFeaturesSelectedOPFFormat(dataset_train, dataset_test, feat_id,
                                         perc, out_train, out_test);
}
...
void Dif(FEMDataset *dataset_train, FEMDataset *dataset_test) {
  ...
  double *dif = (double *)malloc(sizeof(double) *
                                 dataset_train->number_of_features),
         *equals = (double *)malloc(sizeof(double) *
                                    dataset_train->number_of_features),
  ...
  for (i = 0; i < dataset_train->number_of_samples; i++) {
    ...
    for (f = 0; f < dataset_train->number_of_features; f++) {
      value =
          (dataset_train->samples[i].features[f] - min[f]) / (max[f] - min[f]);
  ...
  for (i = 0; i < dataset_test->number_of_samples; i++) {
    ...
    for (f = 0; f < dataset_test->number_of_features; f++) {
      value =
          (dataset_test->samples[i].features[f] - min[f]) / (max[f] - min[f]);
...
double distanceFEM_feature(double a, double b) { return sqrt(pow(a - b, 2.0)); }

double FEMShepardMotherFunction_feature(double a, double b,
                                        double additional_parameters[]) {
  return (1.0 / pow(pow(distanceFEM_feature(a, b) + 0.00000001, 1.0),
                    (double)additional_parameters[0]));
}

double
FEMGaussianNormalizedMotherFunction_feature(double a, double b,
                                            double additional_parameters[]) {
...
double
FEMRadialNormalizedMotherFunction_feature(double a, double b,
                                          double additional_parameters[]) {
  double r = distanceFEM_feature(a, b) / additional_parameters[0];
  return exp(-0.5 * r * r);
}

...
void bubleSortF(double prob[], int index[], int number_of_points)
{
  ...
  for(i = 0; i < number_of_points-1; i++)
  {
	...
	if(prob[i] > prob[j])
	{
	  ...
	  prob[i] = prob[j]; prob[j] = daux;
	  index[i] = index[j]; index[j] = iaux;	  
	} 	  
    }  
  }  
  return;
...
void createDatasetFeaturesSelectedOPFFormat(FEMDataset *dataset_train, FEMDataset *dataset_test, int feat_id[],double perc, char out_train[], char out_test[])
{
    ...
    fprintf(train,"%d %d %d",dataset_train->number_of_samples,dataset_train->number_of_classes,k);
    for(i = 0; i < dataset_train->number_of_samples; i++)
    {
        fprintf(train,"\n%d %d",i+1,dataset_train->samples[i].class);
        for(j = 0; j < k; j++)
        {
            fprintf(train," %f",dataset_train->samples[i].features[feat_id[j]]);
        }
    }
    
    fprintf(test,"%d %d %d",dataset_test->number_of_samples,dataset_test->number_of_classes,k);
    for(i = 0; i < dataset_test->number_of_samples; i++)
    {
        fprintf(test,"\n%d %d",i+1,dataset_test->samples[i].class);
        for(j = 0; j < k; j++)
        {
            fprintf(test," %f",dataset_test->samples[i].features[feat_id[j]]);
        }
    }
...
double FeatureSelectionVector(FEMDataset *dataset_train, FEMDataset *dataset_test, int n_samples, double additional_parameters[], motherFunctionF *FEMbasisF,double perc, char out_train[], char out_test[])
{
    ...
    double *prob_feat_discrepancy = (double*) malloc(sizeof(double)*dataset_train->number_of_features),
           *max                   = (double*) malloc(sizeof(double)*dataset_train->number_of_features),
           *min                   = (double*) malloc(sizeof(double)*dataset_train->number_of_features),
           ***values               = (double***) malloc(sizeof(double**)*dataset_train->number_of_features);
           
    int    *feat_id               = (int*) malloc(sizeof(int)*dataset_train->number_of_features);
    
    if(prob_feat_discrepancy == NULL || feat_id == NULL)
    {
        fprintf(stderr,"\nERROR: FeatureSelectionVector() in module FEMFeatureSelection, cannot allocate vectors!\n");
        exit(1);
    }
    ...
    for(i = 0; i < dataset_train->number_of_features; i++)
    {
        for(k = 0; k < n_samples; k++)
        {
            for(j = 0; j < dataset_train->number_of_classes; j++)
            {
                ...
                values[i][k][j] = probabilityByClassFeature(dataset_train,j+1,i,value,min[i],max[i],additional_parameters,FEMbasisF);
            }
        }
    }
    ...
    for(i = 0; i < dataset_train->number_of_features; i++) //loop feature out
    {
       // fprintf(stdout,"\n%d of %d",i,dataset_train->number_of_features);
        ...
        for(j = i+1; j < dataset_train->number_of_features; j++) //loop feature in
        {                
                for(c1 = 0; c1 < dataset_train->number_of_classes; c1++) //loop class out
                {
                            for(c2 = 0; c2 < dataset_train->number_of_classes; c2++)//loop class in
                            {
                                for(k = 0; k < n_samples; k++)
                                {
  ...
  createDatasetFeaturesSelectedOPFFormat(dataset_train,dataset_test,feat_id,perc,out_train,out_test);     
}
...
void Dif(FEMDataset *dataset_train, FEMDataset *dataset_test)
{
    ...
    double *dif                   = (double*) malloc(sizeof(double)*dataset_train->number_of_features),
           *equals                = (double*) malloc(sizeof(double)*dataset_train->number_of_features),
           *max                   = (double*) malloc(sizeof(double)*dataset_train->number_of_features),
           *min                   = (double*) malloc(sizeof(double)*dataset_train->number_of_features),
           *aux                   = (double*) malloc(sizeof(double)*dataset_train->number_of_features);
    ...
    {
        fprintf(stderr,"\nERROR: Dif() in module FEMFeatureSelection, cannot allocate vectors!\n");
        exit(1);
    }
    ...
    for(i = 0; i < dataset_train->number_of_samples; i++)
    {
        ...
        {
            if(dataset_train->samples[i].class != dataset_train->samples[j].class)
            {
                for(f = 0; f < dataset_train->number_of_features; f++)
                {
                    valuei  =  (dataset_train->samples[i].features[f] - min[f])/(max[f] - min[f]);
                    valuej  =  (dataset_train->samples[j].features[f] - min[f])/(max[f] - min[f]);
    ...
    fprintf(f_dif_train,"%d %d %d",dataset_train->number_of_samples,dataset_train->number_of_classes,dataset_train->number_of_features);
    for(i = 0; i < dataset_train->number_of_samples; i++)
    {
        fprintf(f_dif_train,"\n%d %d",i+1,dataset_train->samples[i].class);
        for(f = 0; f < dataset_train->number_of_features; f++)
        {
            value = (dataset_train->samples[i].features[f] - min[f])/(max[f] - min[f]);
            fprintf(f_dif_train," %f", value*aux[f]);
        }
    }
    
    fprintf(f_dif_test,"%d %d %d",dataset_test->number_of_samples,dataset_test->number_of_classes,dataset_test->number_of_features);
    for(i = 0; i < dataset_test->number_of_samples; i++)
    {
        fprintf(f_dif_test,"\n%d %d",i+1,dataset_test->samples[i].class);
        for(f = 0; f < dataset_test->number_of_features; f++)
        {
            value = (dataset_test->samples[i].features[f] - min[f])/(max[f] - min[f]);
